...
*stretch_robotiq_description* provides materials for a [URDF](http://wiki.ros.org/urdf) kinematic model of the Stretch mobile manipulator from Hello Robot Inc.

## Quick View

```
roslaunch stretch_robotiq_description display.launch
```
...
```
roscd stretch_robotiq_description/urdf
./export_urdf.sh
```
...
1. Edit the [`stretch_robotiq_description.xacro`](./urdf/stretch_robotiq_description.xacro) file in the Stretch Description package.

   Include the tool you want in the `stretch_robotiq_description.xacro`. By default, it will include `stretch_gripper.xacro`. Simply comment out
   ...
   looking at the files matching `stretch_robotiq_description_<tool-name>.xacro`. To add your own custom tool, copy your xacro/mesh files into
